    ...
    <div className='flex justify-center items-center h-screen'>
      <div className='container mt-5'>
        <div className='flex items-stretch justify-between'>
          <button onClick={modalOpen} className='bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4'>
            Registrar
          </button>
          <Input type='text' value={filtroNombre} onChange={handleSearchChange} placeholder='Buscar por nombre' name='filtroNombre' />
          <button onClick={() => setOrderByAlphabet(!orderByAlphabet)} className='bg-yellow-500 hover:bg-yellow-700 text-white font-bold py-2 px-4'>
            Filtrar por orden alfabética {orderByAlphabet ? '(DESC)' : '(ASC)'}
          </button>
          {selectedRows.length > 0 && (
            <button onClick={handleDeleteSelected} className='bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4'>
              ...
              <thead className='text-xs text-gray-900 uppercase bg-green-300 dark:bg-gray-900 dark:text-gray-400'>
                <tr>
                  <th scope='col' className='px-6 py-3 border-green-300'>
                    <input type='checkbox' onChange={handleCheckboxChange} checked={selectedRows.length === usuariosPaginados.length && usuariosPaginados.length !== 0} />
                  </th>
                  <th scope='col' className='px-6 py-3 border-green-300'>
                    ID
                  </th>
                ...
                {usuariosPaginados.map((usuario) => (
                  ...
                  >
                    <td className='px-6 py-4'>
                      <input type='checkbox' checked={selectedRows.includes(usuario.id)} onChange={() => handleRowSelect(usuario.id)} />
                    </td>
                    {editingId === usuario.id ? (
                      <>
                        <td className='px-6 py-4'>{usuario.id}</td>
                        <td className='px-6 py-4'>
                          <Input type='text' onChange={handleInputChangeName} defaultValue={usuario.name} />
                        </td>
                        <td className='px-6 py-4'>
                          <Input type='text' onChange={handleInputChangeSurname} defaultValue={usuario.surname} />
                        </td>
                        <td className='px-6 py-4'>
                          <Input type='text' onChange={handleInputChangeEmail} defaultValue={usuario.email} />
                        </td>
                        <td className='px-6 py-4'>
                          <button onClick={() => handleSubmit(usuario, userEdited)} className='bg-green-900 hover:bg-green-700 text-white font-bold py-2 px-4'>
                            Guardar cambios
                          </button>
                        </td>
                      </>
                      ...
                      <>
                        ...
                        <td>
                          <button onClick={() => handleModificarClick(usuario)} className='bg-blue-500 hover:bg-blue-900 text-white font-bold py-2 px-4'>
                            Modificar
                          </button>
                          <button onClick={() => handleBorrarUsuario(usuario.id)} className='bg-red-500 hover:bg-red-900 text-white font-bold py-2 px-4'>
                            Eliminar
                          </button>
                        </td>
                      </>
        ...
        <div className='flex justify-center mt-4'>
          {[...Array(Math.ceil(usuariosFiltrados.length / usuariosPorPagina)).keys()].map((number) => (
            <button key={number + 1} onClick={() => paginate(number + 1)} className='mx-1 px-3 py-1 border rounded hover:bg-gray-200'>
              {number + 1}
            </button>
          ))}
        </div>
